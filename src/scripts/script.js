...
camera.position.set(3.347, 99, -360);
...
// === HDRI ENVIRONMENT ===
new RGBELoader().load('/hdris/kiara_interior_1k.hdr', (texture) => {
...
cameraFolder
  .add(cameraPos, 'x', -200, 200)
  .onChange((val) => (camera.position.x = val));
cameraFolder
  .add(cameraPos, 'y', -200, 200)
  .onChange((val) => (camera.position.y = val));
cameraFolder
  .add(cameraPos, 'z', -200, 200)
...
targetFolder
  .add(orbitTarget, 'x', -10, 10)
  .onChange((val) => (controls.target.x = val));
targetFolder
  .add(orbitTarget, 'y', -10, 10)
  .onChange((val) => (controls.target.y = val));
targetFolder
  .add(orbitTarget, 'z', -10, 10)
  .onChange((val) => (controls.target.z = val));
targetFolder.open();
